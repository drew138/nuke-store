...
@section('title', __('users.create_user') . ' - ' . __('app.app_name'))
@section('content')
    <div class="w-full p-4 flex flex-col justify-center items-center">
        ...
        <div
            ...
            <form class="space-y-6" method="POST" action="{{ route('admin.users.save') }}" enctype="multipart/form-data">
                @csrf
                <h5 class="text-xl font-medium text-gray-900 dark:text-white">
                    {{ __('users.create_user') }}</h5>

                <div>
                    <label for="name"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('users.name') }}</label>
                    <input type="text" name="name" id="name"
                        placeholder="{{ __('users.name_placeholder') }}" value="{{ old('name') }}"
                ...
                <div>
                    ...
                    <input type="number" name="balance" id="balance"
                        placeholder="{{ __('users.balance_placeholder') }}" value="{{ old('balance') }}"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white">
                </div>
                <div>
                    <label for="email"
