...
@section('content')
    <div class="max-w-7xl h-full mx-auto mb-20 px-4 sm:px-6 lg:px-8">
        <div class="flex  flex-col items-center px-4 pt-4 pb-24 md:p-4 md:flex-row">
            @if ($data['userSession']->getTotalMegatons() > $data['user']->getTotalMegatons())
            ...
            @elseif ($data['userSession']->getTotalMegatons() < $data['user']->getTotalMegatons())
            ...
            @endif
                <div class="flex flex-col items-center">
                    ...
                    <img class="w-24 h-24 mb-3 rounded-full shadow-lg object-cover"
                        src="{{ URL::asset($data['userSession']->getProfilePicture()) }}" />
                    <h5 class="mb-1 text-xl font-medium text-gray-900 dark:text-white">{{ $data['userSession']->getName() }}</h5>
                    <span
                        class="text-sm text-gray-500 dark:text-gray-400">{{ __('users.dictator_of') . ' ' . __('countries.' . $data['userSession']->getCountry()) }}</span>
                </div>
                ...
                <ul class="mx-8 space-y-4 text-left text-gray-500 dark:text-gray-400">
                    @foreach ($data['userSession']->getBombUsers() as $bomb_user)
                ...
                <div class="flex flex-col items-center pb-10">
                    <h1 class="text-3xl mx-8 mb-2 font-extrabold text-gray-900 dark:text-white md:text-4xl lg:text-6xl">
                        @if ($data['userSession']->getTotalMegatons() > $data['user']->getTotalMegatons())
                            <span
                                class="text-transparent bg-clip-text bg-gradient-to-r to-green-800 from-green-600">{{ $data['userSession']->getTotalMegatons() }}</span>
                        @elseif ($data['userSession']->getTotalMegatons() < $data['user']->getTotalMegatons())
                            <span
                                class="text-transparent bg-clip-text bg-gradient-to-r to-red-600 from-red-500">{{ $data['userSession']->getTotalMegatons() }}</span>
                        @else
                            <span
                                class="text-transparent bg-clip-text bg-gradient-to-r to-yellow-600 from-yellow-500">{{ $data['userSession']->getTotalMegatons() }}</span>
                        @endif
                    </h1>
                    @if ($data['userSession']->getTotalMegatons() == 1)
